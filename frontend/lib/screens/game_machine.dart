...
class GameMachine extends ConsumerStatefulWidget {
  final String idRoom;
  const GameMachine({super.key, required this.idRoom});
  @override
    ...
    socket.on('response-on-move', (data) {
      ...
      setState(() {
        if (data['state'] == "ENDGAME") {
          ...
          final last = data['lastTurn'];
          if (last != null) {
            final offset = Offset(last['x'].toDouble(), last['y'].toDouble());

            if (!yourX) {
              visitedX = {...visitedX, offset};
            } else {
              visitedO = {...visitedO, offset};
            }
          }
